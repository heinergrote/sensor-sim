...
- `index.ts` — app bootstrap: creates the simulation service, mounts routes as a flat list (mount order has no
  ...
  Exports `simulationService` (module-level singleton, imported by route handlers) and re-exports everything from
  `types.ts`, including `HonoGlobalVars` (`{user}`, set by `jwtMiddleware`) and `HonoSimVars` (`HonoGlobalVars &
  {sim, simStream}`, set by `withOwnSimMiddleware`) — the `Variables` types each subapp's `Hono<{Variables:
  ...}>()` uses. There is no more `AppType`/typed RPC export; the frontend only imports plain domain types from
  `types.ts`.
...
- `types.ts` — domain types: `Position`, `SimConfig` (adds `ownerId`, `shareToken`), `SimState`, `Simulation`
  (`{ config, state }`), `Status` (`{ startedAt, simListUpdatedAt, numSims }`), `User`/`Profile` (`{id, username,
  admin}`, no `exp`), `JWTPayload` (`{sub, username, admin, exp}`), `HonoGlobalVars` and `HonoSimVars`.
...
- `util/appSecret.ts` — `appSecret()`: reads and validates `JWT_SECRET`, throwing if unset. Shared by
  `middleware/jwtAuth.ts` (login/route JWTs) and `token.service.ts` (share tokens) so both use the same secret
  without duplicating the check.
- `middleware/jwtAuth.ts` — exports `jwtMiddleware`: runs `hono/jwt`'s check (`jwt({secret: appSecret(), alg:
  "HS256"})`), then reads the decoded `jwtPayload` and sets `c.set('user', {id, username, admin})` — everything
  downstream reads `c.get('user')`, not the raw JWT payload. Also exports `wsJwtMiddleware`, which copies a
  `?token=` query param into the `Authorization` header before delegating to `jwtMiddleware` — needed because a
  `WebSocket` upgrade can't set custom headers; apply it only to `/ws` routes so bearer tokens don't otherwise end
  up in URLs.
- `middleware/requireRole.ts` — `requireRole(requireAdmin, {checkDb})`: reads `c.get('user')`, 401 without one, 403
  when admin is required and missing. Assumes `jwtMiddleware` already ran on the same request — always mount it
  after `jwtMiddleware` in a subapp's own `.use()` chain. With `checkDb: true` it re-reads the user from the DB
  instead of trusting the token claim (for fast revocation) — currently unused.
- `middleware/withOwnSim.ts` — `withOwnSimMiddleware(idParamKey = "id")`: assumes `jwtMiddleware` already ran,
  loads the sim named by that route param, 400 with no id, 404 if unknown, 403 if `sim.config.ownerId !==
  user.id`, otherwise sets `sim` (and `simStream`, if the sim is running) in context. Mounted on `sims.ts`'s
  `/:id/*` routes — see "Sharing simulations" below.
...
- `routes/sims.ts` — simulation REST API plus its own per-sim `GET /:id/ws`, and `POST /:id/share`/`unshare`, mounted
  at `/api/sims`, authenticated and (every `/:id*` route) owner-scoped via `withOwnSimMiddleware`.
...
`jwtMiddleware`/`wsJwtMiddleware` live in `middleware/jwtAuth.ts`, `requireRole` in `middleware/requireRole.ts`, and
`withOwnSimMiddleware` in `middleware/withOwnSim.ts` — every subapp imports the same instances rather than
re-deriving them. `jwtMiddleware` also sets `c.set('user', {id, username, admin})` from the decoded payload, so
`requireRole`, `withOwnSimMiddleware` and every handler read `c.get('user')` rather than the raw JWT claims.
`src/index.ts` mounts all of these as a flat list of `.route()` calls — reordering them no longer changes any
endpoint's access level, only which prefix a handler is reached under. `routes/shared.ts` is public in a different
sense: it has no `jwtMiddleware`, but every path is gated by `simShareMiddleware` checking a per-simulation share
token instead (see "Sharing simulations" below). Consequences worth knowing:

- `/api/sims/:id/ws` (live simulation data) needs a bearer token like the rest of `/api/sims`, *and* the caller
  must own that sim — `withOwnSimMiddleware` 403s otherwise. A simulation's owner can share it instead — see below.
- Every other `/api/sims/:id*` route (read, update, delete, start/stop, updateCurrent, share/unshare) is likewise
  owner-scoped; only `GET /` (filtered to the caller's own sims) and `POST /` (create) skip `withOwnSimMiddleware`.
...
`{sub: user.id, username, admin, exp}`, valid 24h. `GET /api/me` echoes `id`/`username`/`admin` from that payload
(no `exp`, since `Profile` doesn't carry it). There is no refresh flow and no server-side session — logout is
purely client-side (drop the token).

## Sharing simulations

`sim_configs.owner_id` is set to the creator's user id at `POST /api/sims` time, and `withOwnSimMiddleware` now
enforces it as a real ACL on every `/api/sims/:id*` route (see "Auth" above) — so the two share endpoints below
don't re-check ownership themselves, they just run after it:

- `POST /:id/share` — calls `token.service.ts#generateToken` for a 7-day token, writes it to
  `sim_configs.share_token` via `simulationService.update`, and returns `{token, expiryDate}`.
...
| Method | Path                 | Body             | Notes                                                                                       |
|--------|----------------------|------------------|---------------------------------------------------------------------------------------------|
| GET    | `/`                  | —                | List the caller's own `Simulation[]`                                                        |
| GET    | `/:id`               | —                | Single `Simulation`; 404 if missing, 403 if not the owner                                   |
...
Every `/:id*` route 404s for an unknown id and 403s if the caller isn't the sim's owner (`withOwnSimMiddleware`);
mutating routes then answer `{success: true}`. There is no separate `updateTarget` endpoint any more — move the
target with `PUT /:id`.
...
`GET /api/me` → `{ id, username, admin }` (the `Profile` shape, no `exp`) from `c.get('user')`, set by
`jwtMiddleware` — no DB read.
...
|-------------------------|--------------------|--------------|---------------------------------------------|------------------------------------------|
| `/api/sims/:id/ws`      | `routes/sims.ts`   | `Simulation` | bearer token (header or `?token=`), owner only | Every position tick while that sim plays |
...
receiving snapshots on open; no request/response — it's push-only. Connecting to an unknown sim id returns 404,
connecting to one you don't own returns 403, and an invalid/expired/stale share token returns 400/403, before the
upgrade.
